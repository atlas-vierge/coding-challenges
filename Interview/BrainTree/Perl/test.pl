...
sub process_add {
    ...
    $sth->execute($cc, $name, 0, $allowance);
...
sub process_charge {
    ...
    my $stmt = qq(SELECT Balance, Allowance FROM CC_Accounts WHERE Name = ?;);
    my $sth  = $dbh->prepare($stmt) or die("DBI Error: Cannot Prepare Stmt\n");
...
sub process_action {
    ...
    elsif ($action =~ /CREDIT/i) { process_credit(); }  
    else { 
